            ...
            if (program.increase){
                if (program.volume){
                    ...
                    if(program.volume == "ALL"){
                        console.log("[VOLUME = ALL | BUYING ENTIRE BALANCE FOR VOLUME]");
                        //get balance of market and sell ALL
                        DUMPIT(program.coin + program.market, program.increase, program.vol);
...
function mainDumpLoop(theMarket, increase, volume){
    //Close all orders, so we can be top of the sell list
			...
			graviex.orderBook(theMarket, function(res){
				if(!res.error){		
					var selling = parseFloat(res.bids[0].price);
                    console.log("[Buying]");
					console.log("Buying  At: " + selling.toFixed(9));
							...
							if(!oursSell){
                                ...
                                clearOrdersForMarket(theMarket, function(res){
                                    if(!res.error){
                                        ...
                                        //Dumping
                                        console.log("Buy Price: " + sellPrice);
                                        //sell
                                        console.log("[ORDER LOG] " + theMarket + " BUY Volume: " + volume + " sellPrice: " + selling);
                                        graviex.createOrder(theMarket, "buy", 1, sellPrice, function(res2){
                                            if(!res.error){
